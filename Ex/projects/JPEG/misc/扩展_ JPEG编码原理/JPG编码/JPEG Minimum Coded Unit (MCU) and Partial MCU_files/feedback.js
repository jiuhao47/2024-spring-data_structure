...
var submitContentFun = function() {
    ...
    $.getJSON(t_CommentHandler.format(queryString), function(response) {
        if (response.Status > 0) {
            ...
            $('#commentslist').append(itemPattern.format(Title, location, response.CommentId, actionUserName, new Date().toLocaleString(), response.ClientIp, Content.HtmlEncode()));
            $("#content").val("");
            $("#ExPwd").val("");
